...
var PotionNames = ["healing_potion", "potion2", "potion3", "potion4"] # what they might order
...
func _ready(): 
	...
	tempCust.setCustomerOrder(PotionNames.pick_random()) 
	
	# generate customers one after another
	#todo
	
	var my_random_number = rng.randf_range(.1, 1.5) # walking speed range
