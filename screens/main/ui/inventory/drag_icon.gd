...
func _get_drag_data(_at_position):
	...
	if inv_data[inv_slot]["Item"] != null:
		...
		find_parent("InventoryDrawer")._get_dragging(inv_slot)
...
func _drop_data(_at_position, data):
	...
	if data["origin_item_id"] == data["target_item_id"] && target_slot != origin_slot:  #if target slot is same item as starting slot, merge the slots
		...
		if Input.is_physical_key_pressed(KEY_SHIFT) : # we are moving whole stack
			if data["target_quantity"] + data["origin_quantity"] > item.stack_size: #if adding this quantity to the amount in the stack would be larger than max stack size
				inv_data[target_slot]["Quantity"] = item.stack_size #fill the target slot
				inv_data[origin_slot]["Quantity"] = data["origin_quantity"] - (item.stack_size -  data["target_quantity"])#leave the remaining in the staring slot
				data["target_slot"].get_node("Icon/Quantity").set_text( str(item.stack_size))
				data["origin_node"].get_node("Quantity").set_text( str(data["origin_quantity"] - (item.stack_size -  data["target_quantity"])))
	...
	elif int(inv_data[origin_slot]["Quantity"]) == int(1) || int(Input.is_physical_key_pressed(KEY_SHIFT)) : #stack only contains one or we are moving whole stack
	...
	elif int(inv_data[target_slot]["Quantity"]) == 0 : # move only one item in larger stack
		inv_data[target_slot]["Item"] = data["origin_item_id"]
		texture = data["origin_texture"]
